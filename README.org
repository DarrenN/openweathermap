...
Load the library from this repository:
#+begin_src lisp
(require :asdf)
...
(let ((current (openweathermap:fetch-current-weather :q "London" :units :metric)))
  (format t "~&City: ~A~%" (getf current :name))
  (format t "Temp (C): ~A~%" (getf (getf current :main) :temp)))
#+end_src

All JSON fetch responses are decoded to plists using normalized keyword keys (for example =:name=, =:weather=, =:dt_txt=).
...
When =OPENWEATHER_API_KEY= is set, live examples are included. Without it, only offline examples run.
...
- Function naming:
  ...
  - For One Call request metadata, =make-onecall-request= is the preferred name; =make-client-weather-request= is retained as a compatibility alias.
...
** Contract Notes
- JSON response objects are normalized to keyword plist keys.
  - Example: use =(getf result :name)=, not escaped key variants.
...
Notes:
- In Lisp, JSON objects are decoded as property lists with normalized keyword keys (for example, use =:name=, =:weather=, =:dt_txt=).
