...
To require running live examples in the loop script, set:
#+begin_src sh
export OPENWEATHERMAP_RUN_LIVE_EXAMPLES=1
#+end_src

For =fetch-current-weather= / =fetch-forecast= examples, keep =:mode= as JSON (or omit it), since fetch helpers decode JSON responses.
...
XDG_CACHE_HOME=$(pwd)/.cache sbcl --script examples/09-live-parser-contract-smoke.lisp
#+end_src
...
Live parser/contract smoke demo (requires =OPENWEATHER_API_KEY=):
#+begin_src sh
XDG_CACHE_HOME=$(pwd)/.cache sbcl --script examples/09-live-parser-contract-smoke.lisp
#+end_src
Runs live calls across Current, Forecast, Geocoding, One Call, and Air Pollution;
verifies normalized plist/keyword decoding and key ergonomic helpers.
